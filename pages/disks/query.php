...
function content_disks_query()
{
 ...
 if (is_int($seg))
 {
  ...
  $segopt['geom'] = (@$pmap[$seg]['type'] == 'geom') ?
   'normal' : 'hidden';
  $segopt['gptfree'] = ($scheme == 'GPT' AND @$pmap[$seg]['type'] == 'free') ? 
   'normal' : 'hidden';
  $segopt['gptboot'] = ($scheme == 'GPT' AND 
   @$pmap[$seg]['type'] == 'freebsd-boot') ? 'normal' : 'hidden';
  $segopt['gptdata'] = ($scheme == 'GPT' AND @$pmap[$seg]['type'] != 'free' AND
   @$pmap[$seg]['type'] != 'freebsd-boot') ? 'normal' : 'hidden';
  $segopt['mbrfree'] = ($scheme == 'MBR' AND @$pmap[$seg]['type'] == 'free') ?
   'normal' : 'hidden';
  $segopt['mbrdata'] = ($scheme == 'MBR' AND @$pmap[$seg]['type'] != 'free') ?
   'normal' : 'hidden';
  ...
  foreach ($segopt as $segmentid => $displaystatus)
   if ($displaystatus == 'normal' AND $segmentid != 'unknown')
    $segopt['unknown'] = 'hidden';
...
//     $segopt[$name] = 'hidden';
   ...
   if ($mounted)
   {
    ...
    foreach ($mp as $data)
     $table_mounteddevices[] = array(
      'MD_CLASS'	=> ($data['device'] == $pmap[$seg]['dev'] OR 
                            $data['device'] == 'gpt/'.@$pmap[$seg]['label']) ?
			    'activerow' : 'normal',
...
//    '/boot/gptzfsboot', false, false, false, 80 * 512));
   ...
   else
   {
    ...
    $sig['expected_mbr'] = @md5(substr(file_get_contents(
     $guru['docroot'].'/files/bootcode/pmbr'), 0, $pmbr_bytecap));
    $sig['expected_bootcode'] = @md5(file_get_contents(
     $guru['docroot'].'/files/bootcode/gptzfsboot', false, false, false, 80 * 512));
    // compare disk signatures to expected signatures
    if ($sig['mbr'] == $sig['expected_mbr'] AND 
     $sig['bootcode'] == $sig['expected_bootcode'])
     $class_gptboot_ok = 'normal';
    else
     $class_gptboot_old = 'normal';
    // compare expected signatures to system signatures
    if ($sig['expected_mbr'] != $sig['system_mbr'] OR 
        $sig['expected_bootcode'] != $sig['system_bootcode'])
     if ((strlen($sig['system_mbr']) > 0) AND 
         (strlen($sig['system_bootcode']) > 0))
...
function submit_disks_segmentprocess()
{
 ...
 elseif (count($pmap) != $_POST['seg_totalseg'])
  error('number of partition segments does not conform to submitted form. '
 ...
 if (@isset($_POST['seg_submit_scheme']))
 {
  ...
  if ($scheme == 'geom')
  {
   ...
   if (!$s)
    friendlyerror('please use only alphanumerical characters in the label name',
     $url2);
   if (file_exists('/dev/label/'.$label))
    friendlyerror('this label is already in use, please choose another one', 
     $url2);
   $result = super_execute('/sbin/glabel create '.$label.' '.$diskname);
   if ($result['rv'] == 0)
   {
    page_feedback('created GEOM label "'.htmlentities($label).'" on disk '
     .$diskname, 'b_success');
    redirect_url($url);
   }
   else
  ...
  if (@$_POST['cb_newscheme_bootpart'] == 'on' AND $scheme == 'gpt')
  {
   ...
   $r = super_execute('/sbin/gpart add -b 64 -s 1024 -t freebsd-boot '
    .$diskname);
   if ($r['rv'] == 0)
   {
    ...
    $r = super_execute($cmd);
    if ($r['rv'] != 0)
     page_feedback('could not insert boot code to boot partition; '
      .'got return value '.$r['rv'], 'a_failure');
   }
   else
    page_feedback('could not create boot partition; got return value '.$r['rv'],
     'a_failure');
  }
  page_feedback('created partition scheme <b>'.strtoupper($scheme)
 ...
 if (@isset($_POST['gpt_seg_submit_free']) OR 
     @isset($_POST['mbr_seg_submit_free']))
 {
  // diskinfo
  ...
  {
   page_feedback('partition size adjusted to '.$maxsize
  ...
  if ($scheme == 'GPT' AND strlen($labelname) > 0)
  {
   $s = sanitize($labelname);
   if (!$s)
    friendlyerror('if you chose to assign a label to this disk, it must consist '
  ...
  if ($scheme == 'GPT')
   $result = super_execute('/sbin/gpart add -t '.$ptype.' -b '.$startoffset
    .' -s '.$size_sectaligned.' '.$gptlabelsuffix.$diskname);
  elseif ($scheme == 'MBR')
   $result = super_execute('/sbin/gpart add -t freebsd -b '.$startoffset
  ...
  if ($trim == 'on')
  {
   ...
   foreach ($pmap as $id => $pdata)
    if ($pdata['size_sect'] == $size_sectaligned)
     if ($devnode === false)
      ...
      friendlyerror('could not TRIM erase newly created partition because '
   ...
   if (strlen($devnodesuffix) > 0)
   {
    ...
    if ($result['rv'] != 0 AND $result['rv'] != 1)
     page_feedback('TRIM erasing disk '.$devnodesuffix.' failed, got return '
      .'value '.(int)$result['rv'].'. Command output:<br /><br />'
      .nl2br($result['output_str']), 'a_failure');
   }
   else
    page_feedback('could not TRIM erase partition; unable to detect device',
     'a_warning');
  }
 }
 ...
 if (@isset($_POST['seg_submit_bootcode']))
 {
  ...
  if ($r['rv'] != 0)
  {
   page_feedback('could not insert boot code to boot partition; '
    .'got return value '.$r['rv'], 'a_failure');
   page_feedback('command output:<br />'.nl2br(htmlentities($r['output_str'])),
    'c_notice');
  }
  else
   page_feedback('updated bootcode on disk <b>'.$diskname.'</b> index '.$index, 
    'b_success');
 }
 ...
 if (@isset($_POST['seg_submit_bootcode_destroy']))
 {
  ...
  if ($result['rv'] == 0)
   page_feedback('destroyed partition with index '.$index.' on '.$diskname,
    'b_success');
  else
   page_feedback('could not destroy partition with index '.$index
 ...
 if (@isset($_POST['submit_destroyscheme']))
 {
  ...
  {
   page_feedback('could not destroy partition scheme on disk '.$diskname,
    'a_failure');
   page_feedback('command output:<br />'
 ...
 if (@isset($_POST['seg_submit_recoverscheme']))
 {
  ...
  if ($result['rv'] == 0)
   page_feedback('successfully recovered partition scheme on '.$diskname, 
    'b_success');
  else
  {
   page_feedback('could not recover partition scheme on disk '.$diskname,
    'a_failure');
   page_feedback('command output:<br />'
    .nl2br(htmlentities($result['output_str'])), 'c_notice');
  }
 ...
 if (@isset($_POST['seg_submit_gpt']) OR @isset($_POST['seg_submit_mbr']))
 {
  ...
  if (@isset($_POST['gpt_seg_label']))
   if (@$_POST['gpt_seg_label'] != @$pmap[$seg]['label'])
   {
    ...
    if (strlen(@$_POST['gpt_seg_label']) < 1)
     $newlabel = '';
    elseif (!$s)
     friendlyerror('if you chose to assign a label to this disk, it must '
      .'consist only of alphanumerical characters, underscore (_) or dash (-)', 
      $url2);
    elseif (file_exists('/dev/gpt/'.$newlabel))
     friendlyerror('this label is already in use, please choose another one',
      $url2);
    $result[] = super_execute('/sbin/gpart modify -i '.$index
     .' -l "'.$newlabel.'" '.$diskname);
  ...
  if ($operation == 'resize')
  {
   ...
   if ($newsize > $pmap[$seg]['size_sect'])
   {
    ...
    if ($pmap[$seg+1]['type'] != 'free')
     friendlyerror('cannot resize because no free space segment is available!',
      $url2);
    // check whether size exceeds the maximum
    if ($size_sectaligned > $maxsize)
    {
     page_feedback('intended partition size ('.$size_sectaligned.' sectors) '
   ...
   // resize partition
   $result = super_execute('/sbin/gpart resize -i '.$index.' -s '
    .$size_sectaligned.' '.$diskname);
   // workaround for FreeBSD bug where device changes are not always propagated
   $result[] = super_script('device_activate_changes', $diskname);
   if ($result['rv'] == 0)
    page_feedback('resized partition with index '.$index.' on '.$diskname,
     'b_success');
   else
    page_feedback('could not resize partition with index '.$index
     .' on '.$diskname, 'a_failure');
  }
  ...
  elseif ($operation == 'zerowrite')
  {
   ...
   if ($result['rv'] != 0 AND $result['rv'] != 1)
    error('Zero writing disk '.$disksegdevsuf.' failed, got return value '
  ...
  elseif ($operation == 'randomwrite')
  {
   ...
   if ($result['rv'] != 0 AND $result['rv'] != 1)
    error('Random writing disk '.$disksegdevsuf.' failed, got return value '
     .(int)$result['rv'].'. Command output:<br /><br />'
     .nl2br($result['output_str']));
  }
  elseif ($operation == 'trimerase')
  {
   ...
   if ($result['rv'] != 0 AND $result['rv'] != 1)
    error('TRIM erasing disk '.$disksegdevsuf.' failed, got return value '
     .(int)$result['rv'].'. Command output:<br /><br />'
     .nl2br($result['output_str']));
  }
 }
