...
function content_disks_advanced() {
	...
	if ( $query ) {
		...
		if ( @is_numeric( $cap[ 'detail' ][ 'Native Command Queuing (NCQ)' ][ 'value' ] {
				0
			} ) )
...
function decode_raw_apmsetting( $rawapm ) {
	...
	if ( ( ( $p = strpos( $rawapm, '/0x80' ) ) != false )AND( is_numeric( @$rawapm {
			$p + 5
		} ) ) )
		return @hexdec( substr( $rawapm, strpos( $rawapm, '/0x80' ) + 5 ) );
	if ( ( strpos( $rawapm, '/0x' ) != false )AND( is_numeric( $rawapm {
			0
		} ) ) )
