  ...
  foreach ($sorted as $diskname => $data)
  {
   ...
   if (@strlen($labels[$diskname]) > 0)
    ...
    else
    {
     ...
     if (file_exists('/dev/'.$diskname) AND 
         !file_exists('/dev/label/'.$labels[$diskname]))
     {
      $nolabeldev = true;
   ...
   if (@strlen($gpart[$diskname]['label']) > 0)
    ...
    else
    {
     ...
     if (file_exists('/dev/'.$diskname) AND 
         !file_exists('/dev/gpt/'.$gpart[$diskname]['label']))
     {
      $nolabeldev = true;
...
function submit_disks_formatdisk()
{
 ...
 if (@$_POST['format_type'] == 'gpt')
 {
  ...
  if ((int)$data_size < (1 * 1024 * 2))
   error('The data partition needs to be at least 1MiB large; '
...
function submit_disks_massprocess()
{
 ...
 if ($action == 'formatgpt')
 {
  ...
  if (!$s OR strlen($labelbase) < 1)
   friendlyerror('please enter a label prefix consisting of alphanumerical, '
 ...
 elseif ($action == 'formatmbr')
 {
  ...
  else
  {
   ...
   page_feedback('could not use <b>zfsboot</b> from webinterface'
    .' - using system image version', 'c_notice');
  }
