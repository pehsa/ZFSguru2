	...
	foreach ( $_POST as $name => $value )
		if ( substr( $name, 0, strlen( 'md_destroy_md' ) ) == 'md_destroy_md' ) {
			...
			if ( @$data[ $mdunit ][ 'backing' ] == 'vnode' ) {
				...
				// nonexistent file
				page_feedback( 'the file that memory disk md' . $mdunit . ' is referring to ('
	...
	if ( @isset( $_POST[ 'md_create' ] ) ) {
		...
		if ( @$_POST[ 'md_type' ] == 'vnode' ) {
			...
			if ( !is_dir( dirname( $file ) ) )
				friendlyerror( 'the path you provided is incorrect; the directory "'
			...
			if ( file_exists( $file ) )
				if ( !is_file( $file ) )
					friendlyerror( 'the file you provided is either a directory or special '
