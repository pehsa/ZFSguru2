...
function content_disks_benchmark()
{
 ...
 elseif ($tabbar_tab == 'advancedbench')
 {
  ...
  if (common_distribution_type() == 'livecd')
   page_feedback('LiveCD detected, ZFS performance is severely restricted '
...
function submit_disks_benchmark_simple()
{
 ...
 // benchmark disk
 $result = super_execute($guru['docroot'].'/scripts/benchmark_simple.php '
  .$disk.' '.$diskinfo['mediasize']
  .' > /dev/null &');
//  );

 page_feedback('benchmark is running!'
...
// advanced benchmark
{
 ...
 foreach ($data['disks'] as $diskname)
 {
  ...
  if ($diskinfo['mediasize'] < 
     (((double)$_POST['testsize_gib'] + 0.01) * 1024 * 1024 * 1024))
   error('disk '.$diskname.' is too small for the chosen test size ('
...
function submit_disks_benchmark_stop()
{
 ...
 // stop processes
 exec('/bin/ps xw | grep "advanced_benchmark.php|raidtest" | grep -v grep', 
  $output, $rv);
 if ($rv == 0)
 {
  $pids = array();
