...
function content_access_samba_shares()
{
 ...
 if (@isset($_GET['removeshare']))
 {
  ...
  if (!@isset($sambaconf['shares'][$_GET['removeshare']]))
   friendlyerror('cannot remove "'.$_GET['removeshare']
 ...
 if (strlen($queryshare) < 1)
 {
  ...
  if (@strpos($_GET['newshare'], '/') !== false)
   $newshare_name = @htmlentities(substr($_GET['newshare'], 
 ...
 elseif (!@isset($sambaconf['shares'][$queryshare]))
  friendlyerror('no share with the name <b>'.htmlentities($queryshare)
   .'</b> exists!', $url);
 else
 {
...
function table_samba_share_accesslist($accesstype, $sharename, $sambaconf)
{
 ...
 if (@is_array($shareperms[$accesstype]))
  foreach ($shareperms[$accesstype] as $name)
  {
   ...
   $uname = ($name == '+share' OR $name == '@share') ? 'Everyone' : $rname;
   $table_accesslist[] = array(
    'SP_TYPE'	=> $type,
...
function submit_access_samba_shares_create()
{
 ...
 foreach ($reservedsharenames as $reservedname)
  if ($sambasharename == $reservedname)
   friendlyerror('you have chosen a reserved name, '
 ...
 if ($mountpoint{0} != '/')
  friendlyerror('incorrect mountpoint supplied; '
 ...
 if ($result AND ($zfsfs == '/mp/'))
  page_feedback('created a new share called <b>'.htmlentities($sambasharename)
 ...
 if (@isset($_POST['submit_sambadeleteshares']))
 {
  ...
  if (count($removed_arr) > 0)
  {
   ...
   if ($result !== true)
    ...
    friendlynotice('removed the following shares: <b>'
...
function submit_access_samba_shares_advanced()
{
 ...
 foreach ($_POST as $postvar => $postvalue)
  if (substr($postvar, 0, strlen('advancedvar_')) == 'advancedvar_')
   ...
   {
    $postvariable = trim(str_replace('_', ' ',
...
function submit_access_samba_shares_dragdrop()
{
 ...
 if ($dragtype == 'standarduser' OR $dragtype == 'useringroup')
 {
  $list_arr = @explode(' ', trim(
   $sambaconf['shares'][$sharename][$conv[$dragtarget]]));
  if (in_array($dragname, $list_arr))
   friendlyerror('user <b>'.htmlentities($dragname).'</b> already part of this '
    .'access list', $redir);
  // remove dragitem from all access lists
  $aclist = array();
  foreach ($conv as $conv_a => $conv_b)
  {
   $tmp_arr = @explode(' ', trim(
 ...
 elseif ($dragtype == 'sambagroup')
 {
  ...
  if (in_array('+'.$dragname, $list_arr) OR in_array('@'.$dragname, $list_arr))
   friendlyerror('group <b>'.htmlentities($dragname).'</b> already part of this '
    .'access list', $redir);
