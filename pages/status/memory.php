...
function content_status_memory() 
{
    ...
    preg_match(
        '/^Mem: ([0-9]+[KMGT]) Active, ([0-9]+[KMGT]) Inact, '
    ...
    $top[ 'cache' ] = @convertunitsize($topmemory[ 5 ]);
    $top[ 'buf' ] = @convertunitsize($topmemory[ 7 ]);
    ...
    // unused
    $topswap[ 'pct' ] = @$swapmemory[ 6 ];

    // sysctl
    $physmem = system_detect_physmem();
    $mem = $physmem[ 'installed' ];
    $kmem = common_sysctl('vm.kmem_size');
    $kmem_max = common_sysctl('vm.kmem_size_max');
    $vmtotal = common_sysctl('vm.vmtotal');
