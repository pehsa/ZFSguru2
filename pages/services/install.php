...
function content_services_install()
{
 ...
 if (@isset($_GET['service']))
 {
  ...
  if ($installed AND !$avail)
   page_feedback('service is installed but database says this service '
...
//   $compatible = guru_checkcompatibility($dist[$svc]['compat']);
  ...
  $ip_class_notinstalled1 = (!$installed AND !$dip AND !$iip AND 
   !$fileavail AND $avail) ? 'normal' : 'hidden';
  $ip_class_notinstalled2 = (!$installed AND !$dip AND !$iip AND 
   $fileavail AND $avail AND $requiresdep) ? 'normal' : 'hidden';
  $ip_class_notinstalled3 = (!$installed AND !$dip AND !$iip AND 
   $fileavail AND $avail AND !$requiresdep) ? 'normal' : 'hidden';
  ...
  // availability chart
  $ac_sys = ''; $ac_version = ''; $ac_size = ''; $ac_quality = '';
...
// checks dependencies required based on DEPEND file contents, returns array
{
