...
function content_system_install_step1() {
	...
	$systemversions = table_systemversions( $system, $locate, $currentver,
...
function table_systemversions( $system, $locate, $currentver, $platform,
	...
	foreach ( $system as $platforms ) {
		...
		$download = server_download_bg_query( server_uri( 'system', $data[ 'filename' ] ),
...
function submit_system_install_download() {
	...
	if ( disk_free_space( $dirs[ 'download' ] ) <
		( @$sys[ 'filesize' ] + $reserved ) )
		if ( $dist == 'livecd' )
