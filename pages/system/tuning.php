...
function content_system_tuning()
{
 ...
 $usablemem = @$physmem['usable'];
 $memory_installed = sizebinary(@$physmem['installed'], 1);
 $memory_usable = sizebinary($usablemem, 1);

 // kmem (kernel memory)
 $kmem = @sizebinary(common_sysctl('vm.kmem_size'), 1);
 $kmem_max = @sizebinary(common_sysctl('vm.kmem_size_max'), 1);
 $kmem_free = @sizebinary(common_sysctl('vm.kmem_map_free'), 1);

 // ARC (adaptive replacement cache)
 $arc_min = @sizebinary(common_sysctl('vfs.zfs.arc_min'), 1);
 $arc_wired = '';
 $arc_max = @sizebinary(common_sysctl('vfs.zfs.arc_max'), 1);
 $arc_min_pct = round(common_sysctl('vfs.zfs.arc_min') / 
  common_sysctl('vm.kmem_size') * 100, 1);
 $arc_max_pct = round(common_sysctl('vfs.zfs.arc_max') / 
  common_sysctl('vm.kmem_size') * 100, 1);
 ...
 $activetuningprofile = loaderconf_activeprofile();
 @$tuning[$activetuningprofile] = 'selected="selected"';
 $tuning_unknown = ($activetuningprofile) ? 'hidden' : 'normal';
 ...
 $tuningvars = array(
  ...
  'vfs.zfs.prefetch_disable' =>
   'disable prefetching for i386 (32-bit) and low-memory systems',
 ...
 return @array(
  ...
  'TUN_INSTALLEDMEM'		=> $memory_installed,
  'TUN_USABLEMEM'		=> $memory_usable,
  'TUN_KMEM'			=> $kmem,
  ...
  'TUNING_PROFILE'		=> $activetuningprofile,
  'TUNING_UNKNOWN'		=> $tuning_unknown,
  'TUNING_NONE'			=> $tuning['none'],
  'TUNING_AGG'			=> $tuning['aggressive'],
  'TUNING_PER'			=> $tuning['performance'],
  'TUNING_BAL'			=> $tuning['balanced'],
