...
function table_pools_poollist()
{
 ...
 foreach ($zpools as $poolname => $pooldata)
 {
  ...
  elseif ($pooldata['status'] == 'FAULTED' OR $pooldata['status'] == 'UNAVAIL')
  {
   $statusclass = 'red pool_faulted';
   if ($class == 'normal')
    $class = 'failurerow pool_faulted';
  }
  elseif ($pooldata['status'] == 'DEGRADED' OR $pooldata['status'] == 'OFFLINE')
  {
...
function submit_pools_importpool()
{
 ...
 if ($result)
 {
  page_feedback('pool imported successfully, it should be visible now!', 
   'b_success');
  redirect_url($url);
 }
