...
function submit_pools_createpool() 
{
    ...
    if ($new_zpool != @$_POST[ 'new_zpool_name' ] ) {
        friendlyerror(
            'please use only alphanumerical characters for the pool name',
            $url 
        );
    }
    if ($new_zpool == '') {
    ...
    if (zfs_pool_isreservedname($new_zpool) ) {
        friendlyerror(
            'you chose a reserved name; please choose a different name!',
            $url 
        );
    }
    ...
    if (in_array($recordsize, array( '256K', '512K', '1024K' )) ) {
        if ($spa == 5000 ) {
            ...
            friendlyerror(
                'pool version 5000 is required for the large_blocks feature',
                $url 
            );
        }
    }
    ...
    foreach ( $vdev[ 'member_disks' ] as $vdevdisk ) {
        ...
        if ($diskinfo[ 'mediasize' ] < $minsize ) {
            friendlyerror(
                'at least one disk selected is too small, minimum is '
    ...
    if (( $vdev[ 'member_count' ] > 1 )AND( $redundancy == '' ) ) {
        page_feedback(
            'you selected RAID0 with more than one disk; are you sure '
