 ...
 if (is_array($pool['members']))
  foreach ($pool['members'] as $id => $member)
  {
   ...
   if ($normaldisk)
   {
    ...
    if (@$member['state'] == 'DEGRADED' OR @$member['state'] == 'OFFLINE')
     $mem_class = 'warningrow member_degraded';
    if (@$member['state'] == 'FAULTED' OR @$member['state'] == 'UNAVAIL')
     $mem_class = 'failurerow member_faulted';
    $mem['online'] = (@$member['state'] == 'OFFLINE') ? 'normal' : 'hidden';
    $mem['offline'] = (@$member['state'] == 'ONLINE' AND $vdevtype != 'stripe')
     ? 'normal' : 'hidden';
    $mem['attach'] = ($vdevtype == 'stripe') ? 'normal' : 'hidden';
    $mem['detach'] = ($vdevtype == 'mirror') ? 'normal' : 'hidden';
    $mem['replace'] = 'normal';
    $mem['clear'] = ((int)@$member['read'] != 0 OR 
                     (int)@$member['write'] != 0 OR 
                     (int)@$member['cksum'] != 0) ? 'normal' : 'hidden';
    $mem['remove'] = ($vdevtype == 'cache' OR 
                      $vdevtype == 'log' OR 
                      $vdevtype == 'hot spares') ? 'normal' : 'hidden';
   }
   else
   {
    ...
    elseif (@$member['state'] == 'DEGRADED' OR @$member['state'] == 'OFFLINE')
     $mem_class = 'warningrow member_degraded';
    elseif (@$member['state'] == 'FAULTED' OR @$member['state'] == 'UNAVAIL')
     $mem_class = 'failurerow member_faulted';
    $mem['online'] = 'hidden';
    $mem['offline'] = 'hidden';
    $mem['attach'] = 'hidden';
