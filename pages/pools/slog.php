...
function content_pools_slog()
{
 ...
 foreach ($zpools as $poolname => $pooldata)
 {
  ...
  // slog specific part
  $class_spa_ok = ($poolspa >= $recommended_spa) ? 'normal' : 'hidden';
  $class_spa_low = ($poolspa < $minimal_spa_version) ? 'normal' : 'hidden';
  $class_spa_warn = ($poolspa >= $minimal_spa_version AND 
                     $poolspa < $recommended_spa) ? 'normal' : 'hidden';
  if ($poolspa > $minimal_spa_version AND $poolspa < $recommended_spa)
  ...
  $poollist[] = array(
   ...
   'POOLLIST_SPA_WARN'		=> $class_spa_warn,
 ...
 if ($querypool)
  if (($zpools[$querypool]['status'] != 'ONLINE') AND 
      ($zpools[$querypool]['status'] != 'DEGRADED'))
 ...
 $memreq = sizebinary($combinedsize / 100, 1);
 $ramsize = common_sysctl('hw.physmem');
 $ramfactor = 8;

 // classes
 $class_moreinfo = (@isset($_GET['explain'])) ? 'normal' : 'hidden';
 $class_step1_spawarn = ($spa_below_recommended) ? 'normal' : 'hidden';
 ...
 $class_step4_single = (count($selecteddisks) == 1) ? 'normal' : 'hidden';
 $class_step4_multi = (count($selecteddisks) > 1) ? 'normal' : 'hidden';
 $class_step4_even = (count($selecteddisks) % 2 == 0) ? 'normal' : 'hidden';
 $class_step4_odd = ((count($selecteddisks) > 1) AND 
                    (count($selecteddisks) % 2 != 0)) ? 'normal' : 'hidden';
 $class_step2_now = ($class_step2 == 'normal' AND $class_step3 != 'normal') ?
  'normal' : 'hidden';
 ...
 return array(
  ...
  'CLASS_STEP1_SPAWARN'	=> $class_step1_spawarn,
  ...
  'CLASS_STEP4_SINGLE'	=> $class_step4_single,
  'CLASS_STEP4_MULTI'	=> $class_step4_multi,
  'CLASS_STEP4_EVEN'	=> $class_step4_even,
  'CLASS_STEP4_ODD'	=> $class_step4_odd,
  'QUERYPOOL'		=> $querypool,
...
function submit_pool_slog()
{
 ...
 if (@isset($_POST['perform_benchmark']) and (count($selecteddevices) > 0))
 {
  ...
  activate_library('background');
  background_register('pool_slog_benchmark', array(
   'commands'	=> $commands,
   'super'	=> true,
  ));
 }
 ...
 if (@isset($_POST['add_slog']))
 {
  ...
  // finally add devices as slog to pool
  $command[] = '/sbin/zpool add '.$pool.' log '
