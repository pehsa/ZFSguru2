...
// installs Root-on-ZFS distribution
{
	...
	exec( '/bin/cp -p /usr/local/etc/smb4.conf ' . $proot . '/usr/local/etc/',
		$output, $rv[] );
...
// installs Root-on-RAM distribution
{
	global $data, $tag, $temp_mp, $source;
...
// installs Root-on-Media distribution
{
	global $data, $tag, $temp_mp, $source;
	// todo
	echo( $tag . 'NOT WORKING YET' . chr( 10 ) );
