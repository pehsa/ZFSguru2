...
// returns array with *SYSTEM* supported ZFS version (zpl+spa)
{
...
// return array of Oracle-exclusive proprietary ZFS features
{
 return array(
...
// detect zpool by looking at zpool list output
{
 ...
 for ($i = 1; $i <= $zpool_count; $i++)
 {
  ...
  if (count($chunks) == 10)
  {
   $zpools[$zpool_name]['frag'] = $chunks[4];
  ...
  elseif (count($chunks) == 7)
  {
   ...
   error('detecting ZFS pools failed; unexpected output from zpool list! '
...
// cached meta function that retrieves zpool status for all pools
{
 global $guru;
...
// returns array of supported ZFS v5000 feature flags
{
...
// retrieves data from 'zpool history' command and returns an array
{
 // elevated privileges
 activate_library('super');
...
// checks if given disk is a member of any ZFS pool and returns its name if so
{
...
// explains for each member disk what its function is, used for pools page
{
...
{
 activate_library('super');
 $result = super_execute('/usr/sbin/zdb -eC '.escapeshellarg($poolname)
  .' | grep ashift');
...
// returns boolean whether provided poolname is acceptable
{
...
// returns boolean on whether $fsname is a ZFSguru system dataset (fs or ZVOL)
{
...
// returns an array of importable pools
{
 // requires root privileges
 activate_library('super');
 ...
 foreach ($searchpaths as $searchpath)
 {
  ...
  foreach ($sp_split as $sp_single)
   if (!@is_dir(trim($sp_single)))
    continue 2;
  $results[$searchpath] = super_execute('/sbin/zpool import '
...
// imports a specified pool(ID); returns boolean
{
 // super privileges
 activate_library('super');
 ...
 switch ($redundancy)
 {
  ...
  case "mirror2":
   if ((int)$member_count % 2 != 0)
    friendlyerror('you chose 2-way mirroring so please select '
     .'a multiple of 2 disks', $url);
   $red = 'mirror2';
   break;
  case "mirror3":
   if ((int)$member_count % 3 != 0)
    friendlyerror('you chose 3-way mirroring so please select '
     .'a multiple of 3 disks', $url);
   $red = 'mirror3';
   break;
  case "mirror4":
   if ((int)$member_count % 4 != 0)
    friendlyerror('you chose 4-way mirroring so please select '
     .'a multiple of 4 disks', $url);
   $red = 'mirror4';
   break;
  case "raidz1":
   if ($member_count < 2)
    friendlyerror('you have chosen RAID-Z1 (single parity) but have selected '
     .'less than two disks. Please select at least 2 disks.', $url);
   $red = 'raidz';
   break;
  case "raidz2":
   if ($member_count < 3)
    friendlyerror('you have chosen RAID-Z2 (double parity) but have selected '
     .'less than three disks. Please select at least 3 disks.', $url);
   $red = 'raidz2';
   break;
  case "raidz3":
   if ($member_count < 4)
    friendlyerror('you have chosen RAID-Z3 (triple parity) but have selected '
