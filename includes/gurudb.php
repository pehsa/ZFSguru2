...
// retrieves a specific item from the GuruDB database
{
 if (!$file)
 {
  global $guru;
...
// validate the downloaded GuruDB database 
{
 global $guru;
 if (!@is_array($newgeneral))
 {
  page_feedback('downloaded GuruDB rejected: invalid array structure',
   'a_failure');
  return false;
 }
 if ($newgeneral['ident'] != 'ZFSGURU:GURUDB')
 {
  page_feedback('downloaded GuruDB rejected: invalid identification',
   'a_failure');
  return false;
 }
 if (!preg_match('/([0-9]+)\:([0-9]+)/', $newgeneral['compat'], $matches))
 {
  page_feedback('downloaded GuruDB rejected: unknown compatibility',
   'a_failure');
  return false;
 }
 elseif ((int)$matches[1] > $guru['compat_max'])
 {
  page_feedback('downloaded GuruDB rejected: database exceeds compatibility - '
   .'update web-interface and try again', 'a_failure');
  return false;
 }
 elseif ((int)$matches[2] < $guru['compat_min'])
 {
  page_feedback('downloaded GuruDB rejected: database is too old (compat)',
   'a_failure');
  return false;
 }
...
// update database by downloading new version if applicable
// return code: true = success/db updated; false = failure; NULL = not modified
{
 global $guru;
 ...
 if ($result === false)
 {
  page_feedback('could not download new GuruDB database from remote servers!', 
   'a_error');
  page_feedback('if you have no internet connection, you can disable remote '
 ...
 elseif ($result !== NULL)
 {
  ...
  exec('/bin/mv '.escapeshellarg(dirname($localdb).'/new.gurudb')
  ...
  page_feedback('installed new GuruDB database, created on: <b>'.$newdate.'</b>', 
   'c_notice');
 }
