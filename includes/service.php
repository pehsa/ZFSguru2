...
// TODO: removed guru -> locate_dirs, make sure dirs are created when needed!
{
	global $guru;
...
// start service and confirm it is actually running; return boolean
{
...
// stop service and confirm it is actually not running anymore; return boolean
{
	// tunables (sleep is in microseconds; 100 000 = 0.1sec)
	$sleep = 100000;
	$runagaincycles = ( int )$runagain_sec * 10;
...
// returns boolean or NULL whether service is configured to automatically start
// TODO: execute as unprivileged user, but requires changes to register script
{
	...
	elseif ( $rv == 6 OR $rv === NULL )
		...
		page_feedback( 'the service_register.sh script returned a strange '
...
// downloads service tarball from one of many slave servers
{
	// required library
	activate_library( 'gurudb' );
...
// returns download status as boolean or integer percentage
{
	// required library
	activate_library( 'gurudb' );
	activate_library( 'server' );
	...
	$status = server_download_bg_query( $uri, $dist[ $svc ][ 'filesize' ],
		$dist[ $svc ][ 'sha512' ] );
...
// installs specified service
{
	...
	if ( filesize( $filepath ) !== $dist[ $svc ][ 'filesize' ] )
		error( 'downloaded file has incorrect size: ' . filesize( $filepath )
			. ', expected: ' . ( int )$dist[ $svc ][ 'filesize' ] );
	if ( trim( shell_exec( '/sbin/sha512 -q '
			. escapeshellarg( $filepath ) ) ) != $dist[ $svc ][ 'sha512' ] )
		error( 'downloaded file fails SHA512 checksum, '
	...
	background_register( $btag, array(
		'commands' => $commands,
		'super' => true,
		'combinedoutput' => true,
	) );
...
// returns string value [notrunning, running, finished] on installation progress
{
	// required library
	activate_library( 'background' );
...
// performs post-installation functions to be run after service installscript
{
	// required libraries
	activate_library( 'background' );
	activate_library( 'super' );
	...
	foreach ( $query[ 'ctag' ] as $name => $data )
		if ( ( $name != 'install' )AND( $data[ 'rv' ] != 0 ) ) {
			page_feedback( 'service ' . htmlentities( $svc ) . ' could not be installed - '
			...
			if ( $data[ 'rv' ] == 1 ) {
				...
				page_feedback( 'service <b>' . $svc . '</b> installed and ready for use!',
					'b_success' );
			elseif ( $data[ 'rv' ] == 3 )
				page_feedback( 'service <b>' . $svc . '</b> installed - '
			...
			elseif ( $data[ 'rv' ] == 4 ) {
				...
				page_feedback( 'service <b>' . $svc . '</b> installed - requires a <b>restart</b> '
					. 'of the webserver; restarting now!', 'b_success' );
			}
			elseif ( $data[ 'rv' ] == 5 ) {
				page_feedback( 'service <b>' . $svc . '</b> failed to install - '
...
// upgrades service to newer version
{
	global $guru;
	...
	if ( filesize( $filepath ) !== $dist[ $svc ][ 'filesize' ] )
		error( 'downloaded file has incorrect size: ' . filesize( $filepath )
			. ', expected: ' . ( int )$dist[ $svc ][ 'filesize' ] );
	if ( shell_execute( '/sbin/sha512 -q '
			. escapeshellarg( $filepath ) ) != $dist[ $svc ][ 'sha512' ] )
		error( 'downloaded file fails SHA512 checksum, '
			. 'it may be corrupted or compromised!' );
	...
	if ( !@file_exists( $installscript ) )
		page_feedback( 'no installation script found when upgrading service',
			'a_warning' );
	else {
		...
		elseif ( $result2[ 'rv' ] == 3 )
			page_feedback( 'service <b>' . $svc . '</b> upgraded - '
		...
		elseif ( $result2[ 'rv' ] == 4 ) {
			...
			page_feedback( 'installation script invalid rv! '
...
// TODO: proper cache drop functionality
{
	global $guru;
...
// checks whether upgrades are available for given service; returns array
{
	// required library
	activate_library( 'gurudb' );
...
// checks whether given process is running and returns boolean or NULL
{
	if ( !$process_name )
		return NULL;
...
// restarts webserver on background with a delay
{
	global $guru;
	// required libraries
	activate_library( 'super' );
	...
	if ( ( strlen( $script ) < 1 )OR( !@file_exists( $script ) ) )
		page_feedback( 'could not restart webserver - please do this manually on '
