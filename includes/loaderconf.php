...
// returns array of memory tuning profiles for loader.conf
{
 return array(
  'none'	=> array(
   'static' => array(
...
// detects active loader.conf memory tuning profile, returns string
{
 // fetch data
 ...
 foreach ($profiles as $profilename => $profile)
 {
  if (@is_array($profile['multiply']))
   foreach ($profile['multiply'] as $multivar => $value)
    if (round($physmem_gib * $value, 1).'g' != $loaderconf[$multivar]['value'] OR
        $loaderconf[$multivar]['enabled'] !== TRUE)
     continue 2;
  if (@is_array($profile['static']))
   foreach ($profile['static'] as $staticvar => $value)
    if (($loaderconf[$staticvar]['value'] != $value OR 
        $loaderconf[$staticvar]['enabled'] !== TRUE) AND
        ($value != FALSE OR $loaderconf[$staticvar]['enabled'] !== FALSE))
     continue 2;
  return $profilename;
 }
 return false;
...
// TODO: outdated, needs updated distribution code
{
 ...
 else
 {
  ...
  page_feedback('unknown memory profile! '
