...
// creates a new background job and starts execution immediately
// NOTE: background_remove should be used prior to registering!
// NOTE: data array should be an array, containing: 
// MANDATORY:
// ['commands'] (array) = array of one or more commands
// OPTIONAL:
// ['execution'] (string) [unprotected/PROTECTED/parallel] = serial or parallel
// ['super'] (bool) [FALSE] = execute as super
// ['combinedoutput'] (bool) [FALSE] = redirect both stdout and stderr to output
// ['storage'] (array) = optional data referenced with the background job
{
 global $guru;
 ...
 if ($query['exists'])
  error('cannot register background job: <b>'
 ...
 $execution = ((@$data['execution'] == 'unprotected') OR
  (@$data['execution'] == 'parallel')) ? $data['execution'] : 'protected';
...
//   error('background: ctag cannot be of type integer');
 ...
 if (!file_put_contents($file_master, $master))
  error('could not write master file: '.htmlentities($file_master)
...
// queries a running or completed background job and returns a rich array
// TODO: [SECURITY] only accept files with owner root
{
 global $guru;
...
// simple query function; returns boolean
{
 $query = background_query($tag);
...
// removes the temporary files of a background job, job may not be running!
// returns boolean whether a job existed or not
{
 global $guru;
 ...
 {
  page_feedback('can not remove a background job which is still running', 
   'a_warning');
  return false;
 }
...
// forcefully stops exection of background job
// TODO
{
