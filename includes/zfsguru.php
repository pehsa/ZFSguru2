...
// returns string with pathname to system image on mounted LiveCD, or false
{
 global $guru;
...
// returns array with pathnames to system images on mounted USB media
{
...
// unmounts LiveCD and any USB media
// TODO: usb
{
 global $guru;
...
// REWRITE!
// setup ZFSguru specific directories for downloads and services
{
 // requires elevated privileges
 activate_library('super');
 ...
 if (!is_link('/download'))
 {
  ...
  $r[] = super_execute('/bin/ln -s '.escapeshellarg('/'.$fs_download)
   .' /download');
 }
 ...
 if (!empty($r))
 {
  ...
  if ($error)
  {
   page_feedback('creation of ZFSguru specific filesystems has failed!',
    'a_error');
   page_feedback('error output: '.$errortxt, 'c_notice');
  }
  else
   page_feedback('ZFSguru specific filesystems have been created on '
...
// returns array with information about availability of system images
{
 // required library
...
// returns string containing SHA512-checksum or boolean false
{
 // call functions
 if (!$locate)
...
// registers a background job that performs a ZFSguru installation
{
 global $guru;
 ...
 background_register('ZFSguru-install', array(
  'commands'		=> $commands,
  'super'		=> true,
  'combinedoutput'	=> true,
 ));
...
// returns array of commands for installation of Root-on-ZFS distribution
{
 global $guru;
...
//  .escapeshellarg($mp_root.'/etc/rc.d/');
 ...
 if (is_int($cam_boot_delay))
  $commands['CONFIGURE-'.$i++] = '/usr/sbin/sysrc -f '.escapeshellarg(
   $mp_root.'/boot/loader.conf').' kern.cam.boot_delay='.(int)$cam_boot_delay;
...
// returns array of commands for installation of Root-on-RAM distribution
{
 viewarray($postdata); die('RoR install');
}

function zfsguru_install_rom($postdata)
// returns array of commands for installation of Root-on-Media distribution
{
 viewarray($postdata); die('RoM install');
...
// returns progress of current install, or boolean false if no installing
{
 // required library
 activate_library('background');
...
// update web-interface with provided tarball, redirect to status page
{
 global $guru;
 ...
 page_feedback('web-interface updated from: '.htmlentities(basename($tarball)),
  'b_success');
 page_feedback('please press F5 or click the refresh page icon - otherwise '
