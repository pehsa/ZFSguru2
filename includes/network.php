...
// checks interface type and returns string
// TODO: wireless interface detection
{
...
// activates new configuration and restarts pf firewall; returns boolean
{
 // required library
 activate_library('super');
...
// checks firewall configuration for errors; returns boolean
{
 // required library
 activate_library('super');
 ...
 if ($r['rv'] == 0)
  return true;
 if (preg_match('~'.preg_quote('/etc/pf.conf:').'([0-9]+)'
  .preg_quote(': syntax error').'~', $r['output_str'], $matches))
  $errorline = (int)$matches[1];
 else
  page_feedback('cannot activate pf firewall due to an error:'
