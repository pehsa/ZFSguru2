...
// provides feedback when downloading has failed
{
 $times = ($invalid != 1) ? ' times' : ' time';
 if ($invalid > 0)
  page_feedback('downloaded file rejected '.$invalid.$times
   .' because the size or checksum is invalid: <b>'
   .htmlentities(basename($uri)).'</b>', 'a_warning');
 else
  page_feedback('tried maximum servers ('.count($failedservers)
...
// tries to download file specified by URL to download directory
{
 ...
 if (($rv == 0) AND @file_exists($downloadpath))
  return $downloadpath;
 if ($rv == 0)
  return NULL; /* IMS: If Modified Since = Not modified since */
 if (@file_exists($downloadpath))
  unlink($downloadpath);
 if ($rv == 1)
...
// download remote file on the background
// TODO: use more fetch parameters like expected filesize
{
 // required library
...
// verifies state of background download and tries other server if required
// return code: false = no download exists; true = download complete; int = pct
// TODO: fetch parameters
{
 ...
 if (($bquery['ctag']['fetch']['rv'] == 0) AND 
     @file_exists($filepath))
 {
  background_remove($btag);
  if (server_validate($filepath, $filesize, $sha512))
  {  
   page_feedback('successfully downloaded: '.htmlentities(basename($uri)), 
    'b_success');
   return true;
  }
  else
  {
   // remove downloaded file that failed validation
   unlink($filepath);
   page_feedback('downloaded file rejected: '.htmlentities(basename($uri)),
    'a_warning');
   return false;
  }
 }
 ...
 if (!@isset($server['address']))
 {
  page_feedback('tried all servers ('.count($failedservers)
 ...
 page_feedback('trying another download server because '.htmlentities($lastserver)
  .' failed to download the file: '.htmlentities(basename($uri)), 'c_notice');
 return 0;
}

function server_uri($type, $filename)
// returns URI for the specified filename and type
{
 // required library
 ...
 if (!$prefix)
  page_feedback('could not retrieve correct URI for download type: '
...
// returns a slave server not listed in $failedservers array
{
 global $guru;
 if (!$serverlist)
 {
...
// checks whether enough space exists to download a file; yields error
{
