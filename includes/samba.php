...
// writes updated samba configuration file to disk
{
	...
	foreach ( $sambaalias as $mastervariable => $aliasvars )
		foreach ( $aliasvars as $aliasvar )
			$split[ 1 ] = preg_replace( '/^[ ]*(' . $aliasvar . ')[ ]*\=[ ]*(.*)$/Um', '',
				$split[ 1 ] );
	foreach ( $invertedalias as $mastervariable => $invertedvars )
		foreach ( $invertedvars as $invertedvar )
			$split[ 1 ] = preg_replace( '/^[ ]*(' . $invertedvar . ')[ ]*\=[ ]*(.*)$/Um', '',
				$split[ 1 ] );
...
// sets Samba-password with given data; requires $username to already exist
{
	// elevated privileges
	activate_library( 'super' );
...
// removes share from configuration
{
...
// removes username from all samba shares
{
	// read samba configuration
...
// removes group from all samba shares
{
...
function samba_usergroups() {
	...
	if ( !empty( $rejectedusers ) )
		page_feedback( 'the following users were rejected because they have a group '
...
// returns string based on access configuration
{
...
// array of variables for the [global] section of smb4.conf
{
	$globalvars = array(
...
// array of variables for the shares section of smb4.conf
{
	$sharevars = array(
...
// array of alias variables that map as <original var> => array(alias1, alias2)
{
	return array(
...
// Samba devs also fancy inverted synonyms, to make things more complicated
{
	return array(
