...
// returns processed source input string
{
 ...
 foreach ($requestedtags as $id => $tag)
   ...
   if (substr($tag, -4) != '_END')
  {
   ...
   $source = preg_replace_callback($preg_table, 'page_callback_tableprocessor', 
    $source);
 ...
 foreach ($requestedtags as $tag)
 {
  ...
  if (is_array($resolved))
   page_feedback('internal error: could not process tag "'.$tag.'" because '
...
// TODO: translation page select
{
 ...
 $prefix = ($pagepath{0} == '/') ? '' : $guru['docroot'];
