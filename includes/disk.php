...
// detect physical disks by ls output and returns an array
{
 ...
 foreach ($disks as $diskid => $disk)
 {
  ...
  if (@(int)$diskinfo['sectorsize'] >= 512)
   if (!$livecd OR ($disk != 'md0' AND $disk != 'md1'))
...
// detect memory disks using mdconfig output
{
 // required libraries
 activate_library('super');
...
// detects disk type and returns string
// TODO: detection of SSD, flash, USB-stick, network
{
 // memory disk
 if (substr($diskname, 0, 2) == 'md')
  return 'memdisk';
...
// returns a partition map array including free space areas
{
 global $guru;
